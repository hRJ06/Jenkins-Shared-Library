def call(Map config = [:]) {
    ...
    withCredentials([usernamePassword(
        ...
        sh """
            sed -i "s|image: hrj06/ecommerce-app:.*|image: hrj06/ecommerce-app:${imageTag}|g" ${manifestsPath}/08-ecommerce-deployment.yml
            
            if [ -f "${manifestsPath}/12-migration-job.yml" ]; then
                sed -i "s|image: hrj06/ecommerce-migration:.*|image: hrj06/ecommerce-migration:${imageTag}|g" ${manifestsPath}/12-migration-job.yml
            fi
